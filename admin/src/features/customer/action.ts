...
export const getListCustomer = async ({
  ...
  try {
    ...
    const url =
      page || limit || filter
        ? `/customer?page=${page}&limit=${limit}&filter=${filter}`
        : "/customer";
...
export const deleteCustomer = async ({
  ...
  try {
    ...
    const accessToken = localStorage.getItem("accessToken");
    const res: IAxiosResponse<User> = await axiosClientJwt.delete(
      `/user/delete/${id}`,
      {
        headers: {
          Authorization: `Bearer ${accessToken}`,
...
export const createCustomer = async ({
  ...
  try {
    ...
    const res: IAxiosResponse<User> = await axiosClientJwt.post(
      "/customer",
      {
        email,
        firstName,
    ...
    if (res?.status === 200 && res?.data) {
      ...
      setError(res?.response?.fieldError as keyof FormValuesCustomer, {
...
export const getCustomer = async ({
  ...
  try {
    ...
    const res: IAxiosResponse<User> = await axiosClientJwt.get(
      `/customer/${id}`,
      {
        headers: {
          Authorization: `Bearer ${accessToken}`,
...
export const updateCustomer = async ({
  ...
  try {
    ...
    if (res?.status === 200 && res?.data) {
      ...
      setError(
        res?.response?.fieldError as keyof Omit<FormValuesCustomer, "password">,
        { message: res?.response?.message },
      );
    } else {
      dispatch(updateCustomerFailed(null));
...
export const getListSearchCustomer = async ({
  ...
  try {
    ...
    const accessToken = localStorage.getItem("accessToken");
    dispatch(getListCustomerStart());
    const res: any = await axiosClientJwt.get(`/customer/search?key=${value}`, {
      headers: {
        Authorization: `Bearer ${accessToken}`,
      },
    });
...
export const getListAddressCustomer = async ({
  params,
  dispatch,
  axiosClientJwt,
  navigate,
  ...
  try {
    ...
    const url =
      page || limit || filter
        ? `/address?page=${page}&limit=${limit}&filter=${filter}`
        : "/address";
    const res: IAxiosResponse<User> = await axiosClientJwt.get(url, {
      headers: {
        Authorization: `Bearer ${accessToken}`,
...
export const createCustomerFast = async ({
  customer,
  axiosClientJwt,
  dispatch,
  navigate,
  ...
  try {
    ...
    error.response.data.phoneNumber
      ? Inotification({
          type: "error",
          message: error.response.data.phoneNumber,
        })
      : Inotification({
          type: "error",
